...
print_results = True #print the results to the screen for each time - turn off for multiprocessing computing
...
save_fn_data = ''.join([save_dir,'secondary_E_',str(grid_density),'X',str(excd_val_low),'excd_',dir,'.npy'])
save_fn_data_csv = ''.join([save_dir,'secondary_E_',str(grid_density),'X',str(excd_val_low),'excd_',dir,'.csv'])

save_fn_vals = ''.join([save_dir,'secondary_E_',str(grid_density),'X',str(excd_val_low),'excd_',dir,'_vals.npy'])
save_fn_vals_csv = ''.join([save_dir,'secondary_E_',str(grid_density),'X',str(excd_val_low),'excd_',dir,'_vals.csv'])
...
if dir == 'east':
    ...
    filelist_fn = ''.join([data_dir,'filelist.npy']) #from BBIDv6
elif dir == 'west':
    bb_data = ''.join(['brightbandsfound_v6_r_6_time0x15.0pcntx25.0_withrhohv_0.910.97_',dir,'.npy'])
    filelist_fn = ''.join([data_dir,'filelist_west.npy']) #from BBIDv6
...
secondary_vals = np.array([1,2,3]) #columns = date, low-mean dBZ, high-mean dBZ
...
def main_func(i):
    ...
    if bb_index == numfiles: #adjust to avoid running past end of the file list
        adj = -1
    else:
        adj = 0
    if bright_bands[bb_index,1] == '1' or bright_bands[bb_index-1,1] == '1' or bright_bands[bb_index+1+adj,1] == '1': #there was a bright band found from bbidv6
        ...
        """
        RADIAL RESTRICITON 10-60 KM FROM RADAR
        """
        ...
        if dir == 'east':
            ...
            n_total = 3525 #applicable for 10-60km

        #allow for selection of +/- 1 time period away from the bright band
        if bright_bands[bb_index,1] == '1':
            bb_ht = (bright_bands[bb_index,2])
        elif bright_bands[bb_index-1,1] == '1':
            bb_ht = (bright_bands[bb_index-1,2])
        elif bright_bands[bb_index+1,1] == '1':
            bb_ht = (bright_bands[bb_index+1,2])
        ...
        if n_found>0:
            ...
            """
            SORT ENHANCEMENT TO MODE LAYERS - HIGH, LOW, OR BOTH & BUILD ENHANCEMENT ARRAYS FOR AVERAGING/PERCENTAGES
            """
            for x_ind in range(x_dim):
                for y_ind in range(y_dim):
                    if modesOK and not big_layer:
                        ...
                        if low_mode-min_sep <= levels[y_ind,x_ind,0] <= low_mode_2+min_sep:
                            levels[y_ind,x_ind,4] = 2 #grid cell's enhancement spans both modes
                            low_enhance_lev.append(levels[y_ind,x_ind,0])
                            high_enhance_lev.append(levels[y_ind,x_ind,1])
                            n_1_found += 1
                            plot_array_1[y_ind,x_ind] =  1 #mark a mode 1 enhancement
                        elif low_mode-min_sep <= levels[y_ind,x_ind,2] <= low_mode_2+min_sep:
                            levels[y_ind,x_ind,5] = 2 #grid cell's enhancement spans both modes
                            low_enhance_lev.append(levels[y_ind,x_ind,2])
                            high_enhance_lev.append(levels[y_ind,x_ind,3])
                            n_1_found += 1
                            plot_array_1[y_ind,x_ind] = 1
        ...
        """
        COLLECT ENHANCEMENT LAYERS AND VALUES FROM THE HIGH DENSITY REGIONS - (ONLY USES CELLS THAT FALL WITHIN HIGH DENSITY REGIONS)
        """
        #enhancement levels
        ...
        for x_ind in range(x_dim):
            for y_ind in range(y_dim):
                if plot_array_1[y_ind,x_ind] > 1 and levels[y_ind,x_ind,4] == 0 or levels[y_ind,x_ind,4] == 2: #enhancement found in grid approach and the enhancement is in the lower mode
                    grid_low_enhance_lev.append(levels[y_ind,x_ind,0])
                    grid_high_enhance_lev.append(levels[y_ind,x_ind,1])
                    grid_enhance_vals.append(np.nanmean(dBZ[0,int(levels[y_ind,x_ind,0]):int(levels[y_ind,x_ind,1]+1), y_ind, x_ind]))
                    #for zz in range(int(levels[y_ind,x_ind,0]),int(levels[y_ind,x_ind,1]+1)):
                        #grid_enhance_vals.append(dBZ[0,zz, y_ind, x_ind])
                elif plot_array_1[y_ind,x_ind] > 1 and levels[y_ind,x_ind,5] == 0 or levels[y_ind,x_ind,5] == 2: #enhancement found in grid approach and the enhancement is in the upper mode
                    grid_low_enhance_lev.append(levels[y_ind,x_ind,2])
                    grid_high_enhance_lev.append(levels[y_ind,x_ind,3])
                    grid_enhance_vals.append(np.nanmean(dBZ[0,int(levels[y_ind,x_ind,2]):int(levels[y_ind,x_ind,3]+1), y_ind, x_ind]))
                    #for zz in range(int(levels[y_ind,x_ind,2]),int(levels[y_ind,x_ind,3]+1)):
                        #grid_enhance_vals.append(dBZ[0,zz, y_ind, x_ind])
                ...
                if plot_array_2[y_ind,x_ind] > 1 and levels[y_ind,x_ind,4] == 1:
                    ...
                    grid_enhance_2_vals.append(np.nanmean(dBZ[0,int(levels[y_ind,x_ind,0]):int(levels[y_ind,x_ind,1]+1), y_ind, x_ind]))
                    #for zz in range(int(levels[y_ind,x_ind,0]),int(levels[y_ind,x_ind,1]+1)):
                        #grid_enhance_2_vals.append(dBZ[0,zz, y_ind, x_ind])
                    ...
                    grid_enhance_2_vals.append(np.nanmean(dBZ[0,int(levels[y_ind,x_ind,2]):int(levels[y_ind,x_ind,3]+1), y_ind, x_ind]))
                    #for zz in range(int(levels[y_ind,x_ind,2]),int(levels[y_ind,x_ind,3]+1)):
                        #grid_enhance_2_vals.append(dBZ[0,zz, y_ind, x_ind])

        """
        AVERAGING VALUES & DATA OUTPUT FOR ENHANCEMENT LAYERS
        """
        if two_lev_enh or (low_lev_enh and high_lev_enh): #two distinct layers
            ...
            mean_low_enhance_lev = np.float64(format(np.nanmean(grid_low_enhance_2_lev)*0.5, '.2f'))
            mean_high_enhance_lev = np.float64(format(np.nanmean(grid_high_enhance_2_lev)*0.5, '.2f'))
            mean_low_enhance_2_lev = float('NaN')
            mean_high_enhance_2_lev = float('NaN')
        else:
    ...
    if plot_figs:
        ...
        """
        if plot_xy and (bright_bands[bb_index,1] ==  '1' or bright_bands[bb_index-1,1] ==  '1' or bright_bands[bb_index+1+adj,1] ==  '1'):
            ...
            ax1.set_xlim([150,215]) if dir =='east' else ax1.set_xlim([85,150])
            ax1.set_ylim([85,215])
            fig.canvas.draw()
            #shift labels over 150 km to center on NPOL
            ax1.set_xticks(np.arange(150,230,20))  if dir =='east' else ax1.set_xticks(np.arange(90,170,20))# choose which x locations to have ticks
            ax1.set_xticklabels(np.arange(0,80, 20))  if dir =='east' else ax1.set_xticklabels(np.arange(60,-20, -20))# set the labels to display at those ticks
        ...
        """
        if  plot_z_fig and (bright_bands[bb_index,1] ==  '1' or bright_bands[bb_index-1,1] ==  '1' or bright_bands[bb_index+1+adj,1] ==  '1'): #there was a bright band found from bbidv6
            ...
            if two_lev_enh or (low_lev_enh and high_lev_enh):
                ...
                lowline = plt.axhline(y=mean_low_enhance_lev*2, color='darkviolet', linestyle='-', label = 'Secondary Enhancement')
                highline = plt.axhline(y=mean_high_enhance_lev*2, color='darkviolet', linestyle='-')
                ax2.add_artist(lowline)
                ax2.add_artist(highline)
    ...
    return(row_to_append, date_secondary_vals)
...
results = pool.map_async(main_func, np.arange(1200,1206))  #testing run
for result in results.get():
    secondary = np.vstack((secondary,result[0]))
    secondary_vals = np.vstack((secondary_vals,result[1]))
...
if save_data:
    ...
    np.save(save_fn_vals, secondary_vals)
    pd.DataFrame(secondary).to_csv(save_fn_data_csv)
    pd.DataFrame(secondary_vals).to_csv(save_fn_vals_csv)
